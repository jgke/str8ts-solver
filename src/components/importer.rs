    ...
    html! {
        <form class="flex border w-full mb-2 flex-col bg-light-800 dark:bg-blue-300 dark:border-blue-400 rounded p-2 max-w-[538px]" {onsubmit}>
            <label class="flex flex-col">
                <span class="block">{"Enter the puzzle. You can either paste a link to the 'default' solver or enter a grid of numbers 1-9, letters a-i, # and . to denote solutions, blockers, walls and holes."}</span>
                <textarea class="mt-4 font-mono tracking-[1em]" rows={10} name="data" />
            </label>
